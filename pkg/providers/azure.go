...
var _ = Describe("Cluster API Azure MachineSet", framework.LabelCAPI, framework.LabelDisruptive, Ordered, func() {
	...
	BeforeAll(func() {
		...
		creator, err := machineset.NewCreator("mapi", "", client)
		if err != nil {
			t.Fatal(err)
		}
	})
	...
	It("should be able to run a machine", func() {
		ms, err := creator.Create(context.Background(), machineset.CreateParams{
			Replicas: 3,
		})
		if err != nil {
			t.Fatal(err)
		...
		t.Logf("Created MAPI MachineSet: %v", ms)
	...
	It("should be able to run a machine with SpotVMOptions", func() {
		...
		// 创建 AWS CAPI MachineSet
		creator, _ := machineset.NewCreator("capi", "azure")
		ms, _ := creator.Create(ctx, machineset.CreateParams{
			Name:     "test-ms",
			Replicas: 1,
			ProviderSpec: &capi.AzureSpec{
