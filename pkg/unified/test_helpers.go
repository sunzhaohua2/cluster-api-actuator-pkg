...
func NewTestHelper(ctx context.Context, uf *framework.UnifiedFramework, client runtimeclient.Client, platform configv1.PlatformType, machineSpec interface{}) *TestHelper {
	return &TestHelper{
		uf:          uf,
		client:      client,
		ctx:         ctx,
...
func (th *TestHelper) CreateTemplate(name string) interface{} {
	tpl, err := th.uf.CreateMachineTemplate(th.ctx, th.client, th.platform, framework.BackendMachineTemplateParams{
		Name:     name,
...
func (th *TestHelper) CreateMachineSet(name string, template interface{}, labels map[string]string) interface{} {
	ms, err := th.uf.CreateMachineSet(th.ctx, th.client, framework.BackendMachineSetParams{
		Name:             name,
		Replicas:         1,
		Labels:           labels,
...
// ReconfigureBackend reconfigures the unified framework backend
func (th *TestHelper) ReconfigureBackend(backendType, authoritativeAPI framework.BackendType) {
	th.uf.ReconfigureBackend(backendType, authoritativeAPI)
}
